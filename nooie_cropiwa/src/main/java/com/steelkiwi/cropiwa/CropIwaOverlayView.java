...
class CropIwaOverlayView extends View implements ConfigChangeListener, OnImagePositionedListener {
    ...
    protected boolean isCropDragEnable = true;
    ...
    protected void notifyNewBounds() {
        ...
        if (mOnBoundsChangeListener != null) {
            RectF rect = new RectF(cropRect);
            mOnBoundsChangeListener.onBoundsChange(rect, getMeasuredWidth(), getMeasuredHeight());
        }
    }
    ...
    public void setCropRect(RectF rect) {
        ...
        if (viewWidth == 0 || viewHeight == 0 || !isCropRectValid(rect, viewWidth, viewHeight) || cropRect == null) {
            return;
        }
        cropRect.set(rect);
    }

    public boolean isCropRectValid(RectF rectF, float viewWidth, float viewHeight) {
        return config != null && rectF != null && !rectF.isEmpty() && (rectF.width() >= config.getMinWidth() && rectF.height() >= config.getMinHeight())
                && (rectF.left >= 0 && rectF.left <= viewWidth - config.getMinWidth()) && rectF.right <= viewWidth
                && (rectF.top >= 0 && rectF.top <= viewHeight - config.getMinHeight()) && rectF.bottom <= viewHeight;
    }

    public void setIsCropDragEnable(boolean enable) {
        isCropDragEnable = enable;
    }

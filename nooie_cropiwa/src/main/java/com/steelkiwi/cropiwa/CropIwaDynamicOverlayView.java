...
class CropIwaDynamicOverlayView extends CropIwaOverlayView {
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        if (!shouldDrawOverlay || !isCropDragEnable) {
            return false;
        }
...
//                (int) cropRect.right, (int) cropRect.bottom);
    ...
    private void updateCornerPointsCoordinates() {
        if (cornerPoints == null || cornerPoints.length < 4 || cropRect == null) {
            return;
        }
    ...
    private void updateCornerPointsCoordinates2() {
        if (cornerPoints == null || cornerPoints.length < 4 || cropRect == null) {
            return;
        }
        cornerPoints[LEFT_BOTTOM].processDrag(cropRect.left, cropRect.bottom);
        cornerPoints[RIGHT_TOP].processDrag(cropRect.right, cropRect.top);
    }
        ...
        if (shouldDrawOverlay) {
            ...
            if (areCornersInitialized()) {
                ...
                for (int i = 0; i < cornerPoints.length; i++) {
                    Log.d("debug", "-->> debug CropIwaDynamicOverlayView onDraw: i=" + i + " x=" + cornerPoints[i].x() + " y=" + cornerPoints[i].y());
                    shape.drawCorner(
    ...
    public void setCropRect(RectF rect) {
        ...
        updateCornerPointsCoordinates();
        updateCornerPointsCoordinates2();
        notifyNewBounds();
        invalidate();
        if (rect != null) {
            Log.d("debug", "-->> debug CropIwaDynamicOverlayView setCropRect: left=" + rect.left + " top=" + rect.top + " right=" + rect.right + " bottom=" + rect.bottom);
        }
    }
}
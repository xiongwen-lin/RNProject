...
public class SwipeMenuLayout extends FrameLayout implements Controller {
    ...
    public SwipeMenuLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        mLeftViewId = typedArray.getResourceId(R.styleable.SwipeMenuLayout_swipeLeftViewId, mLeftViewId);
        mContentViewId = typedArray.getResourceId(R.styleable.SwipeMenuLayout_swipeContentViewId, mContentViewId);
        mRightViewId = typedArray.getResourceId(R.styleable.SwipeMenuLayout_swipeRightViewId, mRightViewId);
        typedArray.recycle();
    ...
    public void smoothCloseMenu(int duration) {
        if (mSwipeCurrentHorizontal != null) {
            ...
            SwipeMenuView rightMenuView = (SwipeMenuView) getChildAt(2);
            if (rightMenuView.getChildAt(0) != null && rightMenuView.getChildAt(0).getTag() instanceof SwipeMenuBridge) {
                SwipeMenuBridge bridge = (SwipeMenuBridge) rightMenuView.getChildAt(0).getTag();
                bridge.reset();
            }
        }
    }

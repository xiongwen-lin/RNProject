...
public class SwipeMenuView extends LinearLayout implements View.OnClickListener {
    ...
    public void createMenu(RecyclerView.ViewHolder viewHolder, SwipeMenu swipeMenu, Controller controller,
        ...
        for (int i = 0; i < items.size(); i++) {
            ...
            boolean isDefaultShowImg = true;
            if (item.getImage() == null) {
                item.setImage(ContextCompat.getDrawable(getContext(), R.drawable.default_icon));
            }
            ...
            if (TextUtils.isEmpty(item.getText())) {
                ...
                isDefaultShowImg = false;
            }
            TextView tv = createTitle(item);
            parent.addView(tv);

            SwipeMenuBridge menuBridge = new SwipeMenuBridge(controller, direction, i, tv, iv, isDefaultShowImg);

...
import com.nooie.widget.touch.DefaultItemTouchHelper;
import com.nooie.widget.touch.OnItemMoveListener;
import com.nooie.widget.touch.OnItemMovementListener;
import com.nooie.widget.touch.OnItemStateChangedListener;
import com.nooie.widget.widget.DefaultLoadMoreView;
 ...
 */
public class NooieSwipeRecyclerView extends RecyclerView {
    ...
    public NooieSwipeRecyclerView(Context context) {
        this(context, null);
    }

    public NooieSwipeRecyclerView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public NooieSwipeRecyclerView(Context context, AttributeSet attrs, int defStyle) {
    ...
    private static class ItemClickListener implements OnItemClickListener {
        ...
        private NooieSwipeRecyclerView mRecyclerView;
        private OnItemClickListener mListener;

        public ItemClickListener(NooieSwipeRecyclerView recyclerView, OnItemClickListener listener) {
    ...
    private static class ItemLongClickListener implements OnItemLongClickListener {
        ...
        public ItemLongClickListener(NooieSwipeRecyclerView recyclerView, OnItemLongClickListener listener) {
            this.mRecyclerView = recyclerView;
            this.mListener = listener;
        }
    ...
    private static class ItemMenuClickListener implements OnItemMenuClickListener {
        ...
        public ItemMenuClickListener(NooieSwipeRecyclerView recyclerView, OnItemMenuClickListener listener) {
            this.mRecyclerView = recyclerView;
            this.mListener = listener;
        }
    ...
    public void smoothCloseRightMenu(int position) {
        smoothCloseMenu(position, RIGHT_DIRECTION, SwipeMenuLayout.DEFAULT_SCROLLER_DURATION);
    }

    public void smoothCloseMenu(int position, @DirectionMode int direction, int duration) {
        if (mOldSwipedLayout != null) {
            if (!mOldSwipedLayout.isMenuOpen()) {
                //mOldSwipedLayout.smoothOpenMenu();
            }
        }
        position += getHeaderCount();
        ViewHolder vh = findViewHolderForAdapterPosition(position);
        if (vh != null) {
            ...
            if (itemView instanceof SwipeMenuLayout) {
                ...
                if (direction == RIGHT_DIRECTION) {
                    mOldTouchedPosition = position;
                    mOldSwipedLayout.smoothCloseMenu(duration);
                } else if (direction == LEFT_DIRECTION) {
                    mOldTouchedPosition = position;
                    mOldSwipedLayout.smoothCloseMenu(duration);
                }
    ...
    public void resetOldSwipedLayout(){

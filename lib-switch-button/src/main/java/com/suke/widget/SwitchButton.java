...
public class SwitchButton extends View implements Checkable {
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        //custom modify 减去的值代表按钮和边框的距离
        buttonRadius = buttonRadius - (buttonRadius/5 + 1);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //custom modify 注释下面代码，防止在修改按钮和边框间距出现间距中间填充
        /*
        drawArc(canvas,
                left, top,
    ...
    private void drawButton(Canvas canvas, float x, float y) {
        //custom modify 通过isChecked改变按钮在关闭和开启的颜色
        if (isChecked) {
            buttonPaint.setColor(Color.WHITE);
        } else {
            buttonPaint.setColor(0XFFC1C1C1);
        }
    ...
    private long timeInterval = 800L;

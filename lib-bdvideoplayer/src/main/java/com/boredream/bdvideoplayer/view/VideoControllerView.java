...
public class VideoControllerView extends FrameLayout {
    ...
    private CustomVideoControlListener mVideoControlListener;

    public void setOnVideoControlListener(OnVideoControlListener onVideoControlListener) {
        this.onVideoControlListener = onVideoControlListener;
    }

    public void setVideoControlListener(CustomVideoControlListener listener) {
    ...
    public void displayStartBtn(boolean show) {
        if (mScreenLock != null) {
            mScreenLock.setVisibility(show ? VISIBLE : GONE);
        }
        mPlayerSeekBar.setEnabled(!show);
    }
    ...
    private int setProgress() {
        ...
        Log.d("tag", "-->> VideoControllerView setProgress position=" + position + " duration=" + duration);
        if (mPlayerSeekBar != null) {
            if (duration > 0) {
                // use long to avoid overflow
                //long pos = 1000L * position / duration;
                //优化结束时pos不为1000，显示异常
                long pos = duration - position > 200 ? 1000L * position / duration : 1000L;
                Log.d("tag", "-->> VideoControllerView setProgress pos=" + pos);
    ...
    private final SeekBar.OnSeekBarChangeListener mSeekListener = new SeekBar.OnSeekBarChangeListener() {
        ...
        public void onProgressChanged(SeekBar bar, int progress, boolean fromuser) {
            ...
            Log.d("tag", "-->> VideoControllerView onProgressChanged progress=" + progress + " duration=" + duration + " mDraggingProgress" + mDraggingProgress);
        ...
        public void onStopTrackingTouch(SeekBar bar) {
            Log.d("tag", "-->> VideoControllerView onStopTrackingTouch mDraggingProgress=" + mDraggingProgress);
    ...
    private void doPauseResume() {
        if (mPlayer.isPlaying()) {
            ...
            if (mVideoControlListener != null) {
                mVideoControlListener.onClickPause();
            }
        } else {
            play();
            displayStartBtn(false);
            if (mVideoControlListener != null) {
                mVideoControlListener.onClickStart();
            }
        }
    }

package com.afar.osaio.routerdata;
...
import com.afar.osaio.smart.device.bean.AESUtils;
...
public class GetRouterDataFromCloud {
    ...
    private String getUrl() {
        String url = "http://" + getRouterIp()/*NooieApplication.routerIp*/ + "/cgi-bin/cstecgi.cgi";
        return url;
    }

    // 获取手机连接路由器ip地址
    private String getRouterIp() {
        WifiManager wifiManager = (WifiManager) NooieApplication.mCtx.getSystemService(Context.WIFI_SERVICE);
    ...
    private String getOnlineUrl() {
        String s3Url = SDKGlobalData.getInstance().getWebUrl();
        String var10000 = NetConfigure.convertBaseUrlWithSlash(s3Url);

        /*String baseUrl = NetConfigure.getInstance().getBaseBrainUrl();

        s3Url = SDKGlobalData.getInstance().getS3Url();
        NetConfigure.getInstance();
        String var10000222 = NetConfigure.convertBaseUrlWithSlash(s3Url);*/

        String url = "http://161.189.16.197:8080/v2/cgi-bin/cstecgi.cgi";
        return url;
    }

    public void getOnlineInitCfg() throws JSONException {
        JSONObject jsonObject = new JSONObject();
        jsonObject.put(TOPICUR,"getInitCfg");
        runOnlineThreadTask(jsonObject);
    }
    ...
    /**
     * 设备需要拥有者
     * device uuid: 30ca5217c72ae9cbb6b96015412e59bd ser: zsuRVlUr   我的 uid : f1729bc79341dc3f
     * @param jsonObject
     */
    private void runOnlineThreadTask(JSONObject jsonObject) {
        String url = getOnlineUrl();
        BindDevice bindDevice = NooieDeviceHelper.getDeviceById("30ca5217c72ae9cbb6b96015412e59bd");
        String uuid = bindDevice.getUuid();
        String deviceSecret = bindDevice.getSecret();
        long timestamp = System.currentTimeMillis()/1000L;

        ApplicationInfo applicationInfo = NooieApplication.mCtx.getApplicationInfo();
        StringBuilder uid = new StringBuilder();
        uid.append(GlobalData.getInstance().getUid())
           .append(uuid)
           .append(timestamp)
           .append(jsonObject.toString());

        String sign = AESUtils.signBASE64(uid.toString(), deviceSecret);
        String plainText = jsonObject.toString();
        byte[] plainTextByte = plainText.getBytes();
        String key = AESUtils.md5Decode32(deviceSecret + timestamp);
        byte[] signByte = sign.getBytes();
        byte[] keyByte = key.getBytes();
        byte[] returnByte = AESUtils.encrypt(plainTextByte, keyByte);

        String returnString = Base64.encodeBase64String(returnByte);

        String topicurlString = "";

        JSONObject jsonObjectString = new JSONObject();
        try {
            topicurlString = jsonObject.getString("topicurl");
            /*jsonObjectString.put("uid",GlobalData.getInstance().getUid());
            jsonObjectString.put("uuid",uuid);
            jsonObjectString.put("timestamp",timestamp);
            jsonObjectString.put("sing",sign);*/
            jsonObjectString.put("param",returnString);
        ...
        SendHttpRequest sendHttpRequest = new SendHttpRequest();
        sendHttpRequest.setRouterReturnInfoListener(from);
        String finalTopicurlString = topicurlString;
        Thread thread=new Thread(){
            public void run(){
                try {
                    sendHttpRequest.sendPostOnline(url, jsonObjectString, GlobalData.getInstance().getUid(), uuid, timestamp, sign, finalTopicurlString);
                } catch (Exception e) {
                    // TODO: handle exception
                }
            }

...
public class WheelView extends View {
    ...
    public final void onItemChanged() {
        if (onItemChangedListener != null && lastSelectedItem != getCurrentItem()) {
            final int index = getCurrentItem();
            final int lastIndex = lastSelectedItem;
            post(new Runnable() {
                @Override
                public void run() {
                    onItemChangedListener.onItemChanged(index, lastIndex);
                }
            });
...
//                canvas.scale(1.0F, (float) Math.sin(radian));
    ...
    private String getContentText(Object item) {
        if (item == null) {
            ...
            //如果为整形则最少保留两位数.
            return String.format(getLocal(context), "%02d", (int) item);
        }
